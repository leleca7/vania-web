-- Vania Work production backend on the existing CURIÓ Supabase project.
-- IMPORTANT: this migration is intentionally compatible with the pre-existing
-- public.profiles table. Do not replace the existing profile policies.

create extension if not exists pgcrypto;

-- Reuse the existing account/profile row and add only the field needed by Vania Work.
alter table public.profiles add column if not exists display_name text;

update public.profiles
set display_name = coalesce(nullif(preferred_name,''), nullif(full_name,''), 'Vania')
where id = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid
...
drop policy if exists "vania_preferences_owner" on public.user_preferences;
create policy "vania_preferences_owner" on public.user_preferences for all to authenticated
using ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid)
with check ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid);

drop policy if exists "vania_opportunities_owner" on public.opportunities;
create policy "vania_opportunities_owner" on public.opportunities for all to authenticated
using ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid)
with check ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid);

drop policy if exists "vania_earnings_owner" on public.earnings;
create policy "vania_earnings_owner" on public.earnings for all to authenticated
using ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid)
with check ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid);

drop policy if exists "vania_monitor_runs_owner" on public.monitor_runs;
create policy "vania_monitor_runs_owner" on public.monitor_runs for all to authenticated
using ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid)
with check ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid);

drop policy if exists "vania_ai_events_select" on public.ai_events;
create policy "vania_ai_events_select" on public.ai_events for select to authenticated
using ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid);
drop policy if exists "vania_ai_events_insert" on public.ai_events;
create policy "vania_ai_events_insert" on public.ai_events for insert to authenticated
with check ((select auth.uid()) = user_id and (select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid);

drop policy if exists "vania_platform_rules_owner_read" on public.platform_rules;
create policy "vania_platform_rules_owner_read" on public.platform_rules for select to authenticated
using ((select auth.uid()) = '330f13da-5421-4256-ab34-6bbd9a94f062'::uuid);

create or replace function public.vania_work_set_updated_at()
returns trigger
language plpgsql
...
$$;
revoke execute on function public.vania_work_set_updated_at() from public, anon, authenticated;

drop trigger if exists vania_preferences_set_updated_at on public.user_preferences;
create trigger vania_preferences_set_updated_at before update on public.user_preferences
for each row execute function public.vania_work_set_updated_at();
drop trigger if exists vania_opportunities_set_updated_at on public.opportunities;
create trigger vania_opportunities_set_updated_at before update on public.opportunities
for each row execute function public.vania_work_set_updated_at();

-- Data API access is explicit on recent Supabase projects. RLS above remains authoritative.
...
values ('330f13da-5421-4256-ab34-6bbd9a94f062'::uuid)
...
set automation_policy=excluded.automation_policy, notes=excluded.notes, updated_at=now();
